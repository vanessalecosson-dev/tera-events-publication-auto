...
add("Générer les Visuels", "n8n-nodes-base.httpRequest", [2640, 0], { method: "POST", url: "={{ ($env.TERA_RENDER_BASE_URL || 'http://tera-render:3000') + '/render/' + $json.template }}", authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendBody: true, specifyBody: "json", jsonBody: "={{ JSON.stringify($json.render_body) }}", options: { timeout: 120000 } }, 4.2);
...
add("Envoyer Pack de Validation", "n8n-nodes-base.emailSend", [3360, 0], { operation: "sendAndWait", fromEmail: "={{ $env.TERA_VALIDATION_EMAIL }}", toEmail: "={{ $env.TERA_VALIDATION_EMAIL }}", subject: "TERA EVENTS - Validation hebdomadaire", message: "Voici les trois publications proposées. Validez, modifiez ou refusez chaque contenu.", responseType: "customForm", defineForm: "json", jsonOutput: "={{ $json.formFieldsJson }}", options: {} }, 2.1);
